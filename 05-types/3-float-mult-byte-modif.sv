module float_mult_byte_modif (
  ...
  always_comb begin
    ...
    if (exponent == 8'd255) begin
      if (mantissa != 0 || sign == 1'b1 || uint8_in == 0) begin
        uint8_out = 8'd0;
      end else begin
        uint8_out = 8'd255;
      end
      ...
      if ((exp_shift - 23) >= 0)
        scaled_mantissa = ((mantissa * uint8_in) << (exp_shift - 23)) + (uint8_in << exp_shift);
      else if (exp_shift >= 0) begin
        mantissa_to_uint = mantissa * uint8_in;
        uint_extended = uint8_in;
        scaled_mantissa = (mantissa_to_uint >> (-(exp_shift - 23))) + (uint_extended << exp_shift);
      end else begin
        mantissa_to_uint = mantissa * uint8_in;
        uint_extended = uint8_in;
